...
```{r}
mod.fit <- glm(formula = admit ~ gre + gpa + rank + gre^2 + gpa^2 + gre * gpa, family = binomial (link = logit), data = df)
mod.fit
```
...
```{r}
mod.fit.h0 <- glm(formula = admit ~  gpa + rank  + gpa^2, family = binomial (link = logit), data = df)
